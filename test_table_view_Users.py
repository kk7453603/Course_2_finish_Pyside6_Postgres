...
class TableEdit(QDialog):
    ...
    def refresh(self):
        ...
        with conn.cursor() as curs:
            ...
            for i in range(len(["fn", "ln", "email", "age"])):
                self.ui.tableWidget.insertColumn(i)
            self.ui.tableWidget.setHorizontalHeaderLabels(["fn", "ln", "email", "age"])
            print(res)
    ...
    def del_row(self):
        ...
        if rowPosition == 0:
            ...
            with conn.cursor() as curs:
                curs.execute(
                    f"DELETE FROM Users WHERE first_name='{self.ui.tableWidget.item(rowPosition - 1, 0).text()}' AND last_name='{self.ui.tableWidget.item(rowPosition - 1, 1).text()}' AND age='{self.ui.tableWidget.item(rowPosition - 1, 3).text()}'")
                conn.commit()
    ...
    def save(self):
        ...
        with conn.cursor() as curs:
            for row in range(self.ui.tableWidget.rowCount()):
                curs.execute(
                    f"SELECT id FROM Users WHERE first_name='{self.ui.tableWidget.item(row, 0).text()}' AND last_name='{self.ui.tableWidget.item(row, 1).text()}';")
                res = curs.fetchone()
                if res:
                    curs.execute(
                        f"UPDATE Users SET first_name='{self.ui.tableWidget.item(row, 0).text()}',last_name='{self.ui.tableWidget.item(row, 1).text()}',email='{self.ui.tableWidget.item(row, 2).text()}', age='{self.ui.tableWidget.item(row, 3).text()}' WHERE id={res[0]};")
                    conn.commit()
                else:
                    curs.execute(
                        f"INSERT INTO Users (first_name,last_name,email,age) VALUES ('{self.ui.tableWidget.item(row, 0).text()}','{self.ui.tableWidget.item(row, 1).text()}','{self.ui.tableWidget.item(row, 2).text()}','{self.ui.tableWidget.item(row, 3).text()}');")
                    conn.commit()
