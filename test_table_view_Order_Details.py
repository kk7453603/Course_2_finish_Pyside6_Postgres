...
class TableEdit(QDialog):
    ...
    def refresh(self):
            ...
            with conn.cursor() as curs:
                ...
                for i in range(len(["date-issue","date-return"])):
                    self.ui.tableWidget.insertColumn(i)
                self.ui.tableWidget.setHorizontalHeaderLabels(["date-issue","date-return"])
                print(res)
    ...
    def del_row(self):
        ...
        if rowPosition == 0:
            ...
            conn = psycopg2.connect(host=host, user=user, password=password, database=db_name)
            with conn.cursor() as curs:
                curs.execute(f"DELETE FROM order_details WHERE date_of_issue='{self.ui.tableWidget.item(rowPosition-1, 0).text()}' AND date_of_return='{self.ui.tableWidget.item(rowPosition-1, 1).text()}';")
                conn.commit()
    ...
    def save(self):
        ...
        with conn.cursor() as curs:
            for row in range(self.ui.tableWidget.rowCount()):
                curs.execute(f"SELECT order_id FROM order_details WHERE date_of_issue='{self.ui.tableWidget.item(row, 0).text()}' AND date_of_return='{self.ui.tableWidget.item(row, 1).text()}';")
                res = curs.fetchone()
                if res:
                    curs.execute(
                        f"UPDATE order_details SET date_of_issue='{self.ui.tableWidget.item(row, 0).text()}',date_of_return='{self.ui.tableWidget.item(row, 1).text()}' WHERE order_id={res[0]};")
                    conn.commit()
                else:
                    curs.execute(
                        f"INSERT INTO order_details (date_of_issue,date_of_return) VALUES ('{self.ui.tableWidget.item(row, 0).text()}','{self.ui.tableWidget.item(row, 1).text()}');")
                    conn.commit()
